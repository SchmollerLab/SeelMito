...
data_path = r'G:\My Drive\01_Postdoc_HMGU\Manuscripts\Anika_mito_2022\NSMB_submission\Revision\microscopy\Mic60\data'

json_paths_filenames = [
    'SCGE_Mic60.json'
    # 'SCD_Mic60.json', 'SCGE_Mic60.json'
]
all_dfs = []
keys = []
...
for json_paths_filename in tqdm(json_paths_filenames, desc='Total', ncols=100):
    ...
    spotmax_df_filename = f'Mic60_del_{medium}_spotMAX_data.csv'
    ...
    for e, exp_folder in tqdm(enumerate(replicates), total=len(replicates), desc='Experiment', ncols=100, position=1, leave=False):
        exp_path = os.path.join(
            data_path, medium, exp_folder, 'TIFFs'
        )
        df_replicate = exp_paths['df_index'][e]
        pos_foldernames = utils.get_pos_foldernames(exp_path)
        for pos in tqdm(pos_foldernames, desc='Positions', ncols=100, leave=False, position=2):
            ...
            for obj in segm_rp:
                ...
                idx = (df_replicate, pos, obj.label)
