...
SCGE_bins = np.arange(25,151,25)

data = SCD_WT_dipl_df # .query('genotype == "WT"')
...
plot.binned_means_plot(
    x=x, y=y, data=data, ax=ax[ax_idx], 
    color=colors[1], bins=SCGE_bins, label='WT',
    scatter_kws=scatter_kws
)

'''Mic60 SCD-SCGE'''
data = SCD_Mic60_df.query('genotype == "WT"')
labels.append('SCD WT (Mic60)')

plot.binned_means_plot(
    x=x, y=y, data=data, ax=ax[ax_idx], 
    color=colors[2], bins=SCD_bins, label='WT',
    scatter_kws=scatter_kws
)
